    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-black text-xl md:text-2xl tracking-tighter text-zinc-900 dark:text-zinc-100 uppercase">
              Email Templates
            </h2>
        ...
        <button
          className="w-full md:w-auto flex items-center justify-center gap-2 px-6 py-4 md:py-3 rounded bg-green-600 text-white font-black text-[10px] uppercase tracking-widest shadow-xl shadow-green-600/20 hover:bg-green-700 transition active:scale-95"
                ...
                {templates.map((t) => (
                  <tr key={t.id} className="hover:bg-green-50/30 dark:hover:bg-green-500/5 transition-all group">
                    <td className="px-8 py-5">
                      <p className="font-black text-zinc-900 dark:text-zinc-100 tracking-tight">{t.name}</p>
                      <p className="text-[10px] text-zinc-400 font-bold truncate max-w-[200px]">{t.subject}</p>
                    </td>
                    <td className="px-6 py-5">
                      <span className="px-3 py-1 rounded bg-zinc-100 dark:bg-zinc-800 text-zinc-500 text-[9px] font-black uppercase tracking-widest border border-transparent group-hover:border-zinc-200 transition-all">
                        {t.category}
                      </span>
                    </td>
                    <td className="px-6 py-5">
                      <span className="px-3 py-1 rounded bg-green-50 dark:bg-green-500/10 text-green-700 dark:text-green-500 text-[9px] font-black uppercase tracking-widest border border-green-100 dark:border-green-900/20">
                        {t.status}
                      </span>
                    </td>
                    <td className="px-6 py-5 text-[10px] font-black text-zinc-400 uppercase">
            ...
            {templates.map((t) => (
              <div key={t.id} className="bg-white dark:bg-zinc-900 rounded-md border border-zinc-100 dark:border-zinc-800 p-6 space-y-4 shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-500">
                <div className="flex justify-between items-start">
                  ...
                  <span className="px-3 py-1 rounded bg-green-50 dark:bg-green-500/10 text-green-600 text-[8px] font-black uppercase tracking-tighter border border-green-100 dark:border-green-900/20">
                    {t.status}
                  </span>
                </div>
                
                <p className="text-xs text-zinc-500 line-clamp-1 font-medium">Subject: {t.subject}</p>
          ...
          <div className="fixed inset-0 z-[120] flex items-end md:items-center justify-center md:p-4 bg-black/60 backdrop-blur-md">
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 300 }} className="bg-white dark:bg-zinc-950 rounded-t-[2.5rem] md:rounded-[3rem] shadow-2xl w-full max-w-5xl h-[92vh] md:h-[85vh] overflow-hidden flex flex-col border-t md:border dark:border-zinc-800">
              <div className="p-6 border-b dark:border-zinc-800 flex justify-between items-center bg-zinc-50/80 dark:bg-zinc-900/80 backdrop-blur-xl">
                <div>
                  <h3 className="font-black text-xl tracking-tighter uppercase text-zinc-900 dark:text-zinc-100">{viewTemplate.name}</h3>
                ...
                <button onClick={() => setViewTemplate(null)} className="h-12 w-12 flex items-center justify-center bg-zinc-200 dark:bg-zinc-800 rounded transition-all active:scale-90"><X size={20} /></button>
              </div>
